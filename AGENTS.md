...
builds and still uses the moment up, `--dry-run` writes nothing, `--no-fresh` skips drawing new
mascot art (generation is on by default — invariant 2). Kill switch: `SS_HALT=1` or a file at
`state/HALT`.
...
2. **Three mascot paths. Only one of them runs at build time.**
   - **The runtime path, and the fallback**: about 190 poses in `mascot/library/` and growing,
