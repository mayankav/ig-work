...
def save(deck, writer, outcome, score, reason, objections):
    if outcome != 'publish': raise ValueError('The whole-deck review did not pass')
    ...
    record = {'version': 1, 'contract': contract(), 'markdown_sha256': hashlib.sha256(markdown.encode()).hexdigest(),
              ...
              'outcome': outcome, 'score': score, 'reason': reason, 'objections': objections}
...
def validate(deck):
    try:
        ...
        markdown = (Path(deck) / 'carousel.md').read_text()
        if (record['version'] != 1 or record['contract'] != contract() or record['outcome'] != 'publish'
