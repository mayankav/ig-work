...
2. **Three mascot paths. Only one of them runs at build time.**
   - **The runtime path, and the only one**: 180 full-body poses in `mascot/library/`,
     ...
     network, deterministic. A build that needs a network is a build that can fail at 8am with
     nobody watching, which is why the other two paths are offline tools and never callable
     from `build.py`.
   ...
   All three read the SAME plain-English `**Mascot:**` brief, so a script written for one works
   with the others. Prompts are in `mascot/GENERATION_PROMPTS.md`. Pose metadata is data, not
