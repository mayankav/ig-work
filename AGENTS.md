...
.agents/skills/suresilly-carousel/   the engine
  ...
  tests/            30 suites, CI runs every one
...
state/              what has been used. Committed. `gate_faults.jsonl` is the
                    ledger of which check refused a run — written by run.py,
                    read only by the report
...
| 4 | **Gates abort, never warn — and the one exception is typed by a person, per run, into a deck that cannot post.** If a gate fails, nothing is written. Reword the brief instead. On 2026-09-03 both slots produced nothing twice over, and the deck the owner never saw was three faults from finished: `write_deck` kept `best_copy` and dropped it on the floor. So the owner may reply `force`, which returns that best draft, lists every fault it still carries, and **holds** it — a second, separate `publish <id>` is the only thing that posts it. An unattended run is byte-identical to before; `allow_faults` defaults to `False` and `test_message.py` pins that. What `force` may reach is short **by construction**, not by a list: everything genuinely dangerous already runs somewhere else — the citation in `bibliography` before the loop, the safety screen at moment selection, novelty in `compose`, both claim locks before drafting, the pixel gates at render. Of the checks inside `verify_draft`, exactly two are not taste and `hard_faults` refuses them under `force` as well: `check_leak`, because a person saying "post it anyway" cannot make a sentence stop being lifted from our own prompt, and `check_mascots`, because the other end of that override is lettering on the mascot. |
...
| 29 | **An alert says what happened, what you can reply, and what silence does — or it is not an alert.** On 2026-09-03 both slots sent "A gate refused, which is the system working. Reply retry" — every word a constant in `auto-post.yml`, while the real reason sat unread on `run.py`'s stdout. The owner replied `retry`, it ran seven minutes and hit the identical wall, because the gate was mathematically unsatisfiable. Three defects, one message: the reason was thrown away, a verb was offered that could not work, and the ideal template already existed on exactly one path (`hold_for_review`) and had never been given to the other three. So there is **one** template in `dashboard.py` and every status renders through it. Colour is emoji, because Telegram HTML has `<b>`, `<i>`, `<code>` and no colour: `MARK` is the key and `test_message.py` fails any message printing a pictograph outside it — a key that is only a convention is one edit from being wrong. `retry` is withheld on a stuck trail and **marked 🚫 rather than removed**, since a verb hidden is a verb the owner goes hunting for. `STUCK_AFTER=3` is measured on the three real trails: `6,2,1` and `4,4,4,2,1` converged and finished, `13,5,4,3,3,3,3` did not — and the pin is on the **prefix** a live loop sees, `[4,4]` converging and `[4,4,4]` stuck, because every finished trail reads the same at 2, 3 and 4 and a mutation walks straight past them. The wording is held to invariant 21 by pointing `readability.line_faults` at the template's own sentences; the one exception is the quoted bad example, which is how the owner learns which word. **A held message is a caption, and a caption that overflows loses its reply list** — Telegram caps it at 1024 and `notify.py` spills the tail into a second message, cutting near the end, which is exactly where the replies are. The old `[:6]` cap on objections could never have been right, because what fits depends on wrapping, on HTML escaping and on the reviewer's note: six short objections fit and two long ones do not. So `_fit_held` renders and measures rather than counting, dropping the tail and saying how many it dropped, down to the bare summary line if it has to — the floor that matters is the reply list, not the objections. Dropping objections is not enough on its own: a realistic critic paragraph rendered at **1101** characters, and no number of dropped objections brings back a note. `NOTE_BUDGET=140` is the second lever and is measured on the rendered floor — 145 fits, 153 is over — and is pinned from both sides, at the cap and one past it, because a trimmer that only ever subtracts is as wrong as one that never does. |

## Failed-slot recovery

A claimed slot is not a confirmed post. Use the explicit `recover` workflow input
with the exact `slot_id`; never delete a claim to unblock work. Pre-build recovery
requires changed code and reruns all checks. A built deck is restored from its
original artifact. An uncertain post is checked against Instagram and may only
reuse its original container. Held decks still require a separate publish decision.
New blocked manual requests get an alert; duplicate clock deliveries stay quiet.
Telegram receipts and retries never control Instagram posting. Pool refill has
its own daily check and a committed 24-hour cooldown. See `docs/slot-recovery.md`.
